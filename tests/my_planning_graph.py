...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            pgNode_a = PgNode_a(action)  # create those PgNode_a objects
            # to see if a proposed pgNode_a has prenodes taht are a subset of the previous S level
            prenodes = pgNode_a.prenodes
            s_literals = set()
            for i in self.s_levels[level]:
                if i in prenodes:
                    s_literals.add(i)
            if prenodes.issubset(s_literals):
                self.a_levels[level].add(pgNode_a)
            if prenodes.issubset(s_literals):
                for parent in s_literals:
                    parent.children.add(pgNode_a)  # connecting child to parent in graph
                    pgNode_a.parents.add(parent)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        count = 0
        effA = node_a1.effnodes
        effB = node_a2.effnodes
        for a in effA:
            for b in effB:
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        effA = node_a1.effnodes
        effB = node_a2.effnodes
        precondA = node_a1.prenodes
        precondB = node_a2.prenodes
        for a in effA:
            for b in precondB:
                if a.symbol==b.symbol and a.is_pos != b.is_pos:
                    flag1 = 1
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        flag = 0
        precondA = node_a1.parents
        precondB = node_a2.parents
        for a in precondA:
            for b in precondB:
                if a.is_mutex(b) or b.is_mutex(a):
                    flag = 1
                    break
            if flag:
                break
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        flag = 1
        parent_actionA = node_s1.parents
        parent_actionB = node_s2.parents
        for a in parent_actionA:
            for b in parent_actionB:
                if not a.is_mutex(b):
                    flag = 0
                    break
        if flag:
